...
def exp(transform=None,adapt=False):
    ...
    else: datafile = kfood(args['path'],False)
    ...
    if args['init_model']:
        list1 = []
        for _,_,files in os.walk(os.path.join('/workspace/results',args['project'],args['tags'])):
            for file in files:
                list1.append(int(file.split('_')[-1].split('.')[0]))
                
        list1.sort(reverse=True)
        model.load_state_dict(torch.load(os.path.join('/workspace/results',args['project'],args['tags'],f'model_{list1[0]}.pt'))['model'])
